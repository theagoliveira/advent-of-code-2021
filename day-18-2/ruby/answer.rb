...
class Answer
  ...
  def add_all(numbers)
    total = Marshal.load(Marshal.dump(numbers.first))

    numbers[1..-1].each do |number|
      # puts "\n"
      # puts "  #{total}"
      # puts "+ #{number}"
      total = add(total, Marshal.load(Marshal.dump(number)))
  ...
  def main
    ...
    permutations.size.times do |index|
      # puts "\nindex: #{index}"
      # puts "\nnumber_pair: #{permutations[index]}"

      total = add_all(permutations[index])
