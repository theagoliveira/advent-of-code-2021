...
class Answer
  ...
  def main
    ...
    boards.each do |board|
        ...
        next unless !winning_boards.include?(board) && check(board)

        if winning_boards.size < boards.size - 1
      ...
      break if last_winning_board.any?
    end

    score = last_winning_board.reduce(0) do |board_sum, row|
